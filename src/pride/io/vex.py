...
VexContent: TypeAlias = MultiDict
...
VEX_PARSER: yacc.LRParser = yacc.yacc(debug=False, write_tables=False)


def parse(s) -> VexContent:
    return VEX_PARSER.parse(s, lexer=lexer)


def Vex(file) -> VexContent:
    fp = open(file, "r")
    vex = fp.read()
    fp.close()
