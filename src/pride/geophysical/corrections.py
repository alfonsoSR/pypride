...
class SolidTide(Displacement):
    ...
    def calculate(self, station: "Station", epochs: "time.Time") -> None:
        ...
        # Get station position at epochs
        coords = np.array(station.location(epochs).geocentric).T

        # Get epochs as ET
        et = (epochs.tdb - J2000.tdb).sec  # type: ignore

        # Get position of Sun, Earth and Moon wrt SSB in GCRS
        sun_gcrs = spice.spkpos("sun", et, "J2000", "NONE", "earth")[0]
        moon_gcrs = spice.spkpos("moon", et, "J2000", "NONE", "earth")[0]
